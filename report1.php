	...
	if($_SESSION['auth']){
		if($_POST){
			...
			if(!isset($_POST['select_all'])){
				...
				if($_POST['whom'] == "current"){
					if($typo == 'regular'){
						$query = $query.", lab_full_name FROM equip_lab,equipment,supplies,vendor,lab WHERE equip_lab.lab_id = $lab_id AND equip_lab.eqp_id = equipment.eqp_id AND equipment.eqp_id = supplies.euip_id AND supplies.vendor_id = vendor.vendor_id AND equipment.del_flag = 1 AND equip_lab.lab_id = lab.lab_id AND lab.lab_id = $lab_id AND purchase_date BETWEEN '$from_date' AND '$to_date'";
					} else if($typo == 'write_off'){
						$query = $query.", lab_full_name FROM equip_lab,equipment,supplies,vendor,lab WHERE equip_lab.lab_id = $lab_id AND equip_lab.eqp_id = equipment.eqp_id AND equipment.eqp_id = supplies.euip_id AND supplies.vendor_id = vendor.vendor_id AND equipment.del_flag = 0 AND equip_lab.lab_id = lab.lab_id AND lab.lab_id = $lab_id AND purchase_date BETWEEN '$from_date' AND '$to_date'";
						...
						$query = $query.",from_lab_name,to_lab_name,transfer_date FROM transfer,equipment,supplies,vendor WHERE from_lab_id = $lab_id AND equip_id = eqp_id AND eqp_id = euip_id AND vendor.vendor_id = supplies.vendor_id AND del_flag = 1 AND purchase_date BETWEEN '$from_date' AND '$to_date'";
					}
				}
				else if($_POST['whom'] == "all"){
					if($typo == 'regular'){
						$query = $query.", lab_full_name, FROM equip_lab,equipment,supplies,vendor,lab WHERE equip_lab.eqp_id = equipment.eqp_id AND equipment.eqp_id = supplies.euip_id AND supplies.vendor_id = vendor.vendor_id AND equip_lab.lab_id = lab.lab_id AND equipment.del_flag = 1 AND purchase_date BETWEEN '$from_date' AND '$to_date'";
					} else if($typo == 'write_off'){
						$query = $query.", lab_full_name FROM equip_lab,equipment,supplies,vendor,lab WHERE equip_lab.eqp_id = equipment.eqp_id AND equipment.eqp_id = supplies.euip_id AND supplies.vendor_id = vendor.vendor_id AND equip_lab.lab_id = lab.lab_id AND equipment.del_flag = 0 AND purchase_date BETWEEN '$from_date' AND '$to_date'";
					}
					else if($typo == 'summary'){
						$query = $query.",lab_full_name, count(name_of_equip) as quantity FROM vendor, supplies, equipment, lab, equip_lab where equip_lab.eqp_id = equipment.eqp_id AND vendor.vendor_id = supplies.vendor_id AND equipment.eqp_id = supplies.euip_id AND equipment.del_flag = 1 AND equip_lab.lab_id = lab.lab_id AND purchase_date BETWEEN '$from_date' AND '$to_date' GROUP BY name_of_equip, supplies.bill_no";
					} 
					else{
						$query = $query.",from_lab_name,to_lab_name,transfer_date FROM transfer,equipment,supplies,vendor WHERE equip_id = eqp_id AND eqp_id = euip_id AND vendor.vendor_id = supplies.vendor_id AND del_flag = 1 AND purchase_date BETWEEN '$from_date' AND '$to_date'";
					}
				}
				...
				while($row = $result->fetch_assoc()){
					...
					if(isset($_POST['name_of_equip'])){
						...
						$output .= '<td>'.$row["name_of_equip"].'</td>';
					}
					if(isset($_POST['quantity'])){
					...
					if(isset($_POST['central_dsr_no'])){
						$output .= '<td>'.$row["central_dsr_no"]."/".$row["cdr"].'</td>';
				...
				if($_POST['whom'] == "current"){
					if($typo == 'regular'){
						$query = "SELECT lab_full_name,name_of_equip,quantity,type,warranty,rate,amount,central_dsr_no,cdr,description,purchase_date,supply_date,equipment.bill_no,purchase_order_no,name_of_vendor,contact_no,address FROM equip_lab,equipment,supplies,vendor,lab WHERE lab.lab_id=equip_lab.lab_id AND equip_lab.lab_id = $lab_id AND equip_lab.eqp_id = equipment.eqp_id AND equipment.eqp_id = supplies.euip_id AND supplies.vendor_id = vendor.vendor_id AND equipment.del_flag = 1 AND purchase_date BETWEEN '$from_date' AND '$to_date'";
					} else if($typo == 'write_off'){
						...
						$query = "SELECT name_of_equip,quantity,type,warranty,rate,amount,central_dsr_no,cdr,description,purchase_date,supply_date,equipment.bill_no,purchase_order_no,name_of_vendor,contact_no,address,from_lab_name,to_lab_name,transfer_date FROM transfer,equipment,supplies,vendor WHERE from_lab_id = $lab_id AND equip_id = eqp_id AND eqp_id = euip_id AND vendor.vendor_id = supplies.vendor_id AND del_flag = 1 AND purchase_date BETWEEN '$from_date' AND '$to_date'";
					}
				}
				
				if($_POST['whom'] == "all"){
					if($typo == 'regular'){
						$query = "SELECT lab_full_name,name_of_equip,quantity,type,warranty,rate,amount,central_dsr_no,cdr,description,purchase_date,supply_date,equipment.bill_no,purchase_order_no,name_of_vendor,contact_no,address FROM equip_lab,equipment,supplies,vendor,lab WHERE lab.lab_id=equip_lab.lab_id AND equip_lab.eqp_id = equipment.eqp_id AND equipment.eqp_id = supplies.euip_id AND supplies.vendor_id = vendor.vendor_id AND equipment.del_flag = 1 AND purchase_date BETWEEN '$from_date' AND '$to_date'";
					} else if($typo == 'write_off'){
						$query = "SELECT lab_full_name,name_of_equip,quantity,type,warranty,rate,amount,central_dsr_no,cdr,description,purchase_date,supply_date,equipment.bill_no,purchase_order_no,name_of_vendor,contact_no,address FROM equip_lab,equipment,supplies,vendor,lab WHERE lab.lab_id=equip_lab.lab_id AND equip_lab.eqp_id = equipment.eqp_id AND equipment.eqp_id = supplies.euip_id AND supplies.vendor_id = vendor.vendor_id AND equipment.del_flag = 0 AND purchase_date BETWEEN '$from_date' AND '$to_date'";
					} 
					else if($typo == 'summary'){
						$query = "SELECT name_of_equip,type,warranty,rate,amount,central_dsr_no,cdr,description,equipment.bill_no,supplies.purchase_date,supplies.supply_date,supplies.bill_no,name_of_vendor,contact_no,address,lab_full_name, count(name_of_equip) as quantity ,purchase_order_no FROM vendor, supplies, equipment, lab, equip_lab where equip_lab.eqp_id = equipment.eqp_id AND vendor.vendor_id = supplies.vendor_id AND equipment.eqp_id = supplies.euip_id AND equipment.del_flag = 1 AND equip_lab.lab_id = lab.lab_id AND purchase_date BETWEEN '$from_date' AND '$to_date' GROUP BY name_of_equip, supplies.bill_no";
					}
					else{
						$query = "SELECT name_of_equip,quantity,type,warranty,rate,amount,central_dsr_no,cdr,description,purchase_date,supply_date,equipment.bill_no,purchase_order_no,name_of_vendor,contact_no,address,from_lab_name,to_lab_name,transfer_date FROM transfer,equipment,supplies,vendor WHERE equip_id = eqp_id AND eqp_id = euip_id AND vendor.vendor_id = supplies.vendor_id AND del_flag = 1 AND purchase_date BETWEEN '$from_date' AND '$to_date'";
					}
				}
